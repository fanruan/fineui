...
(function (mod) {
    ...
    NativeScrollbars.prototype = copyObj({
        ...
        overlayHack: function () {
            ...
            var barMouseDown = function (e) {
                if (e_target(e) != self.vert && e_target(e) != self.horiz) {operation(self.cm, onMouseDown)(e);}
            };
    ...
    function updateWidgetHeight (line) {
        if (line.widgets) {
            for (var i = 0; i < line.widgets.length; ++i) {line.widgets[i].height = line.widgets[i].node.offsetHeight;}
        }
    }
    ...
    function applyTextInput (cm, inserted, deleted, sel, origin) {
        ...
        if (paste && sel.ranges.length > 1) {
            if (lastCopied && lastCopied.join("\n") == inserted) {multiPaste = sel.ranges.length % lastCopied.length == 0 && map(lastCopied, splitLines);} else if (textLines.length == sel.ranges.length) {multiPaste = map(textLines, function (l) { return [l]; });}
        }
    ...
    TextareaInput.prototype = copyObj({
        ...
        poll: function () {
          ...
          isReadOnly(cm) || cm.options.disableInput || cm.state.keySeq) {return false;}
        ...
        "; outline: none; border-width: 0; outline: none; overflow: hidden; opacity: .05; filter: alpha(opacity=5);";
            ...
            function rehide () {
                ...
                te.selectionEnd > 0 && input.prevInput == "\u200b") {operation(cm, commands.selectAll)(cm);} else if (i++ < 10) display.detectingSelectAll = setTimeout(poll, 500);
    ...
    ContentEditableInput.prototype = copyObj({
        init: function (display) {
            ...
            on(div, "input", function () {
                ...
                if (!input.pollContent()) {runInOp(input.cm, function () {regChange(cm);});}
            });
        ...
        showPrimarySelection: function () {
          ...
          cmp(maxPos(curAnchor, curFocus), prim.to()) == 0) {return;}
            ...
            try { var rng = range(start.node, start.offset, end.offset, end.node); } catch(e) {} // Our model of the DOM might be outdated, in which case the range we try to set can be impossible
            if (rng) {
                sel.removeAllRanges();
                sel.addRange(rng);
        ...
        startGracePeriod: function () {
            ...
            this.gracePeriod = setTimeout(function () {
                ...
                if (input.selectionChanged()) {input.cm.operation(function () { input.cm.curOp.selectionChanged = true; });}
            }, 20);
        },
        ...
        receivedFocus: function () {
            var input = this;
            if (this.selectionInEditor()) {this.pollSelection();} else {runInOp(this.cm, function () { input.cm.curOp.selectionChanged = true; });}
        ...
        applyComposition: function (composing) {
            if (composing.data && composing.data != composing.startData) {operation(this.cm, applyTextInput)(this.cm, composing.data, 0, composing.sel);}
        },
        ...
        function walk (node) {
            if (node.nodeType == 1) {
                ...
                if (markerID) {
                    ...
                    if (found.length && (range = found[0].find())) {text += getBetween(cm.doc, range.from, range.to).join("\n");}
                    return;
                }
        ...
        do {
            ...
            for (var j = 0; j < group.ops.length; j++) {
                ...
                if (op.cursorActivityHandlers) {
                    while (op.cursorActivityCalled < op.cursorActivityHandlers.length) {op.cursorActivityHandlers[op.cursorActivityCalled++](op.cm);}
                }
            }
    ...
    function endOperation_W2 (op) {
        ...
        if (op.preparedSelection) {cm.display.input.showSelection(op.preparedSelection);}
    ...
    function endOperation_finish (op) {
        ...
        if (unhidden) {
            for (var i = 0; i < unhidden.length; ++i) {if (unhidden[i].lines.length) signal(unhidden[i], "unhide");}
        }
    ...
    function registerEventHandlers (cm) {
        ...
        } else {on(d.scroller, "dblclick", function (e) { signalDOMEvent(cm, e) || e_preventDefault(e); });}
    ...
    function gutterEvent (cm, e, type, prevent, signalfn) {
        try { var mX = e.clientX, mY = e.clientY; } catch(e) { return false; }
        ...
        if (files && files.length && window.FileReader && window.File) {
            ...
            try {
                ...
                if (text) {
                    ...
                    if (selected) {
                        for (var i = 0; i < selected.length; ++i) {replaceRange(cm.doc, "", selected[i].anchor, selected[i].head, "drag");}
                    }
    ...
    function makeChange (doc, change, ignoreReadOnly) {
        ...
        if (split) {
            for (var i = split.length - 1; i >= 0; --i) {makeChangeInner(doc, {from: split[i].from, to: split[i].to, text: i ? [""] : change.text});}
        } else {
    ...
    CodeMirror.prototype = {
        ...
        moveH: methodOp(function (dir, unit) {
            ...
            cm.extendSelectionsBy(function (range) {
                if (cm.display.shift || cm.doc.extend || range.empty()) {return findPosH(cm.doc, range.head, dir, unit, cm.options.rtlMoveVisually);}
        ...
        setSize: methodOp(function (width, height) {
            ...
            cm.doc.iter(lineNo, cm.display.viewTo, function (line) {
                if (line.widgets) {
                    for (var i = 0; i < line.widgets.length; i++) {if (line.widgets[i].noHScroll) { regLineChange(cm, lineNo, "widget"); break; }}
                }
                ++lineNo;
            });
    ...
    function conflictingCollapsedRange (doc, lineNo, from, to, marker) {
        ...
        if (sps) {
          ...
          fromCmp >= 0 && (cmp(found.from, to) < 0 || (sp.marker.inclusiveLeft && marker.inclusiveRight))) {return true;}
            }
        }
    }
    ...
    var off = CodeMirror.off = function (emitter, type, f) {
        if (emitter.removeEventListener) {emitter.removeEventListener(type, f, false);} else if (emitter.detachEvent) {emitter.detachEvent("on" + type, f);} else {
            ...
            if (!arr) return;
            for (var i = 0; i < arr.length; ++i) {if (arr[i] == f) { arr.splice(i, 1); break; }}
        }
    };
